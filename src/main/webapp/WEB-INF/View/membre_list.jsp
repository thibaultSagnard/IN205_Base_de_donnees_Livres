	      ...
	      <table class="striped no-padding">
            ...
            <tbody>
				...
				<c:if test="${!empruntsCourant.isEmpty()}">             
                    	<c:forEach items="${membresCourant}" var="membre">
                    		<tr>
                       			<td>${membre.getNom()}</td>
                          		<td>${membre.getPrenom()}</td>
                          		<td class="hide-on-small-only"> ${membre.getAdresse()}</td>
                          		<td class="hide-on-small-only"> ${membre.getMail()}</td>
                          		<td class="hide-on-small-only"> ${membre.getTelephone()}</td>
                				<td class="center"><a href="membre_details?id=${membre.getIdPrimaryKey()}"><ion-icon class="details" name="information-circle-outline"></ion-icon></a></td>
                    		</tr>
                    	</c:forEach>
                   	</c:if>
